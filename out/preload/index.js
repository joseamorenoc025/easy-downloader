...
const api = {
  ...
  extractMetadata: (url) => electron.ipcRenderer.invoke("extract-metadata", url),
  ...
  setNotifications: (enabled) => electron.ipcRenderer.invoke("set-notifications", enabled),
  ...
  setCookiesPath: (path) => electron.ipcRenderer.invoke("set-cookies-path", path),
  ...
  checkFileExists: (path) => electron.ipcRenderer.invoke("check-file-exists", path),
